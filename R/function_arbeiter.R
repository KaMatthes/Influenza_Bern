function_arbeiter <- function(Type) {

data_arbeiter <- read.csv("../data_raw/Fabrik_Statistik_1929.csv", sep=";")  %>%
...
glmNB <- glm.nb(Sum_date ~Arbeiter_prop+offset(log(Population)),data = data_inc, link = "log")
summary(glmNB)
}
...
plot_co <- ggplot(data=data_inc) +
  ...
  geom_smooth(aes(x= Arbeiter_prop,y= Sum_Inc),  method='lm',se=TRUE,lwd=lwd_size, col=col_line) +
