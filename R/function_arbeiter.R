function_arbeiter <- function(Type,pandemic_start, pandemic_end,Pandemic_Name) {
  
data_arbeiter <- read.xlsx("../data_raw/Fabrik_Statistik_1929.xlsx")  %>%
...
data_inc <- data_bern %>%
  ...
  mutate(Sum_Inc =Sum_date/Population*1000,
         ...
         # Arbeiter_prop=ifelse(Arbeiter_prop>50,50,Arbeiter_prop),
         Arbeiter_group2= ifelse(Arbeiter==0, 0, 1),
         Arbeiter_group2 = as.factor(Arbeiter_group2),
         Arbeiter_group= as.factor(cut(Arbeiter_prop, c(-Inf, 0, 1, 5, 15, Inf))))
         

if(Type=="Regression") {
# poisson2 <- glm(Sum_date ~ TB_mor+offset(log(Population)),data = data_inc,  family=poisson)
glmNB_r <-  robmixglm(Sum_date ~  Arbeiter_prop+offset(log(Population)),data = data_inc, family="nbinom")
summary(glmNB_r)
...
plot_co <- ggplot(data=data_inc) +
  geom_point(aes(x= Arbeiter_prop, y= Sum_Inc), lwd=lwd_size_points ) +
  geom_smooth(aes(x= Arbeiter_prop,y= Sum_Inc),  method="rlm",se=TRUE,lwd=lwd_size, col=col_line) +
  # ggtitle(" Relation Altitude vs Influenza")+
  ggtitle(Pandemic_Name)+
  ylab("Incidence per 1'000 inhabitants")+
  xlab("Total workers per 1'000 inhabitants") +
  theme_bw() +
...
# cowplot::save_plot("output/plot_SEP.pdf",plot_SEP,ba
  ...
  bezirk_geo <- read_sf("../data_raw/Gemeindegeometrie/Gemeinden_BE_1918.shp") %>%
    ...
    select(GEM_ID, geometry,Sum_Inc,Arbeiter_prop)
  
  bezirk_geo <-as(bezirk_geo, "Spatial")
  
  bivariate_choropleth(bezirk_geo, c("Arbeiter_prop", "Sum_Inc"), c("Worker", "Influenza"), bivmap_title="Relation workers and Influenza")
