...
# cowplot::save_plot("output/plot_SEP.pdf",plot_SEP,ba
  ...
  bezirk_geo <- read_sf("../data_raw/Gemeindegeometrie/Gemeinden_BE_1918.shp") %>%
    ...
    select(GEM_ID, geometry,Sum_Inc,Lws_prop)
  
  bezirk_geo <-as(bezirk_geo, "Spatial")
  
  bivariate_choropleth(bezirk_geo, c("Lws_prop", "Sum_Inc"), c("Agriculture", "Influenza"), bivmap_title="Relation working in agriculture and Influenza")
