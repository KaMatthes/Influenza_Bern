...
data_gew <- read.csv("../data_raw/Cofactors_1918.csv", sep=";")  %>%
  ...
  mutate( Gemeinde_Name = recode( Gemeinde_Name,"St. Beatenberg" ="Beatenberg",
                                  ...
                                  "Rüti" = "Rüti b. Büren")) %>%
  select(GEM_ID,Gemeinde_Name, Gew) 
...
# cowplot::save_plot("output/plot_SEP.pdf",plot_SEP,ba
  ...
  bezirk_geo <- read_sf("../data_raw/Gemeindegeometrie/Gemeinden_BE_1918.shp") %>%
    ...
    select(GEM_ID, geometry,Sum_Inc,Gew_prop)
  
  bezirk_geo <-as(bezirk_geo, "Spatial")
  bivariate_choropleth(bezirk_geo, c("Gew_prop", "Sum_Inc"), c("Industry", "Influenza"), bivmap_title="Relation working in industry and Influenza")
