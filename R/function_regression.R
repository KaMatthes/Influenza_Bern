function_regression <- function(pandemic_start, pandemic_end, Pandemic_Name){
  ...
  data_stations <- read_sf("data_raw/railway/railway_stations_bern.shp") %>%
...
results_regression <- rbind(Mod1, Mod2,Mod3, Mod4, Mod5, Mod6, Mod7, Mod8, Mod9) %>%
  ...
  mutate(est= round(exp(Estimate),5),
         ...
         OR = paste0(est," (",Cl,"-",Cu, ")"),
         Pandemic = Pandemic_Name, Cofactor=recode(Cofactor, 
                                                   "dens_grouplarge" = "Population density (large)",
                                                   "HöheüM" = "Metres above sea level",
                                                   "TB" = "Tuberculosis",
                                                   "Betriebe_prop" = "Share factories",
                                                   "Lws_prop" = "Share agriculture",
                                                   "Gew_prop" = "Share industrie",
                                                   "Arbeiter_prop" = "Share blue collar",
                                                   "PS_prop" = "Share hp",
                                                   "stations_groupyes" = "Train station dummy (yes)"))%>%
  select(Pandemic,Cofactor, OR)
