...
glmNB <- glm.nb(Sum_date ~ H?he?M+offset(log(Population)),data = data_inc, link = "log")
summary(glmNB)
}

else if(Type=="Figure") {
plot_co <- ggplot(data=data_inc) +
  geom_point(aes(x=H?he?M, y= Sum_Inc), lwd=lwd_size_points ) +
  geom_smooth(aes(x=H?he?M,y= Sum_Inc),  method='lm',se=TRUE,lwd=lwd_size, col=col_line) +
