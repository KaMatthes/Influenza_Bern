function_inla_fitted <- function(Year_Inf){
  ...
  mean.samples <- Data %>%
    ungroup() %>%
    dplyr::select(starts_with("V"),"Year", "Region", "Num","Population") %>%
    # group_by(Region) %>%
    # summarise_all(sum) %>%
    rowwise(Region) %>% 
    ...
    select(Region, fit, LL, UL, Num, Population, Year) %>%
    arrange(Region) %>%
...
function_inla_fitted(Year_Inf=1918)
function_inla_fitted(Year_Inf=1919)
function_inla_fitted(Year_Inf=1920)
function_inla_fitted(Year_Inf=1921)
function_inla_fitted(Year_Inf=1922)
function_inla_fitted(Year_Inf=1923)
function_inla_fitted(Year_Inf=1924)
function_inla_fitted(Year_Inf=1925)
function_inla_fitted(Year_Inf=1926)