function_R_effectiv_all <- function(pandemic_start,  pandemic_end ) {
...
data_region <- data_bern %>%
  ...
  mutate(roll_num =rollmean(NumRegion,3, na.pad=TRUE, align="right"))
         # roll_num  = ifelse(roll_num<6,0, roll_num))
...
std_serial_interval = 1
  ...
  # 
# return(  res_R_eff)
save(res_R_eff,file=paste0("data/R_eff_data_all/res_R_eff_all_",pandemic_start,".RData"))
  
}

function_R_effectiv_all(pandemic_start="1918-06-28",  pandemic_end="1919-06-20")
function_R_effectiv_all(pandemic_start="1920-01-09",  pandemic_end="1920-06-18")
function_R_effectiv_all(pandemic_start="1922-01-06",  pandemic_end="1922-04-21")
function_R_effectiv_all(pandemic_start="1924-01-04",  pandemic_end="1924-06-06")
function_R_effectiv_all(pandemic_start="1924-11-21",  pandemic_end="1925-05-08")



path <- "data/R_eff_data_all"
setwd(path)
files <- list.files("C:/Users/kmatth/Dropbox/Influenza_Bern/data/R_eff_data_all")
results <- sapply(files, function(x) mget(load(x)), simplify = TRUE) 
res_R_eff_all <-   as.data.frame(do.call(rbind,   results)) %>%
  select(-start)

save(res_R_eff_all,file=paste0("res_R_eff_all.RData"))
