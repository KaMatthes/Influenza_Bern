...
glmNB <- glm.nb(Sum_date ~  Total_rain+offset(log(Population)),data = data_inc, link = "log")
summary(glmNB)
}
  ...
  plot_co <- ggplot(data=data_inc) +
    ...
    geom_smooth(aes(x= Total_rain,y= Sum_Inc),  method='lm',se=TRUE,lwd=lwd_size, col=col_line) +
