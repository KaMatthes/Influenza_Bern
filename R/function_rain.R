function_rain <- function(Type,pandemic_start, pandemic_end,Pandemic_Name) {

data_rain <- read.csv("../data_raw/Cofactors_1918.csv", sep=";")  %>%
  ...
  mutate( Gemeinde_Name = recode( Gemeinde_Name,"St. Beatenberg" ="Beatenberg",
                                  ...
                                  "Rüti" = "Rüti b. Büren")) %>%
  select(GEM_ID,Gemeinde_Name, Total_rain) 
...
glmNB_r <-  robmixglm(Sum_date ~  Total_rain+offset(log(Population)),data = data_inc, family="nbinom")
summary(glmNB_r)
...
return(res)
  ...
  plot_co <- ggplot(data=data_inc) +
    geom_point(aes(x= Total_rain, y= Sum_Inc), lwd=lwd_size_points ) +
    geom_smooth(aes(x= Total_rain,y= Sum_Inc),  method='rlm',se=TRUE,lwd=lwd_size, col=col_line) +
    # ggtitle(" Relation Precipitation vs Influenza")+
...
# cowplot::save_plot("output/plot_SEP.pdf",plot_SEP,ba
  ...
  bezirk_geo <- read_sf("../data_raw/Gemeindegeometrie/Gemeinden_BE_1918.shp") %>%
    ...
    select(GEM_ID, geometry,Sum_Inc,Total_rain)
  
  bezirk_geo <-as(bezirk_geo, "Spatial")
  bivariate_choropleth(bezirk_geo, c("Total_rain", "Sum_Inc"), c("Precipitation", "Influenza"), bivmap_title="Relation Precipitation and Influenza")
