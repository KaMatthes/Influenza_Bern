data_household <- read.csv("data_raw/Cofactors_1918.csv", sep=";")  %>%
  ...
  mutate( Gemeinde_Name = recode( Gemeinde_Name,"St. Beatenberg" ="Beatenberg",
                                  ...
                                  "Rüti" = "Rüti b. Büren")) %>%
  select(GEM_ID,Gemeinde_Name,Haush) 
# %>%
...
data_inc <- data_bern %>%
  ...
  left_join(data_household) %>%
  mutate(Sum_Inc =Sum_date/Population*1000,
         House_nr = Population/Haush,
         House_nr = as.factor(cut(House_nr,c(0,4,5,Inf), right=TRUE, include.lowest=TRUE ,
                                  label=c("[0-4]", "(4-5]", "5 and more")))) %>%
  filter(!is.na(House_nr))



# poisson2 <- glm(Sum_date ~ TB_mor+offset(log(Population)),data = data_inc,  family=poisson)
glmNB <- glm.nb(Sum_date ~   House_nr+offset(log(Population)),data = data_inc, link = "log")
summary(glmNB)



plot_household <- ggplot(data=data_inc,aes(x=House_nr, y=Sum_Inc)) +
  geom_violin(aes(x=House_nr, y=Sum_Inc))+
  # geom_boxplot(data=datared, aes(factor(Grippe), weight),width=.1) +
  geom_quasirandom(aes(x=House_nr,y=Sum_Inc), width = 0.3,size=1, alpha=0.5) +
  stat_summary(fun = median,
               ...
               geom = "crossbar",
               show.legend=TRUE) +
  ggtitle("Relation between persons per household and incidence")+
  ylab("Incidence per 1'000 inhabitants")+
  xlab("Persons per household") +
  theme_bw() +
  theme(
