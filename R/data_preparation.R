gemeinde <- read.xlsx("data_raw/Data_Population.xlsx",detectDates = TRUE, sheet="Gemeinden") 


data_s <- read.xlsx("data_raw/Data_Spanishflu.xlsx", sheet="Rohdaten",detectDates = TRUE) %>%
  rename(Year = Jahr,
         Med_DIN_Week = Date,
         NumbCases =Number,
...
data_bern <- read.xlsx("data_raw/Data_Influenza_Bern.xlsx",detectDates = TRUE) %>%
  rename(Year=Jahr,
         NumbCases=Number,
         Gemeinde_Name =  Gemeinde) %>%
  mutate(Wochennummer = isoweek(Date)) %>%
  complete(Wochennummer,  Gemeinde_Name, nesting(Year)) %>%
