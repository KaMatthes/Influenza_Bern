...
data_inc <- data_bern %>%
  ...
  mutate(Sum_Inc =Sum_date/Population*1000,
         ...
         PS_group= as.factor(cut(PS, c(-Inf, 0, 20, 80, 400, Inf))),
         PS_group2= ifelse(PS==0, 0, 1),
         PS_group2 = as.factor(PS_group2))
         

if(Type=="Regression") {
# poisson2 <- glm(Sum_date ~ TB_mor+offset(log(Population)),data = data_inc,  family=poisson)
glmNB <- glm.nb(Sum_date ~   PS+offset(log(Population)),data = data_inc, link = "log")
summary(glmNB)
}

else if(Type=="Figure") {
plot_co <- ggplot(data=data_inc) +
  geom_point(aes(x= PS_prop, y= Sum_Inc), lwd=lwd_size_points ) +
  geom_smooth(aes(x=  PS_prop,y= Sum_Inc),  method='lm',se=TRUE,lwd=lwd_size, col=col_line) +
  ggtitle(" Relation PS vs Influenza")+
  ylab("Incidence per 1'000 inhabitants")+
  xlab("Proportion  of PS") +
  xlim(0,8) +
  theme_bw() +
...
# cowplot::save_plot("output/plot_SEP.pdf",plot_SEP,ba
  ...
  bezirk_geo <- read_sf("../data_raw/Gemeindegeometrie/Gemeinden_BE_1918.shp") %>%
    ...
    select(GEM_ID, geometry,Sum_Inc,PS_prop)
  
  bezirk_geo <-as(bezirk_geo, "Spatial")
  
  bivariate_choropleth(bezirk_geo, c("PS_prop", "Sum_Inc"), c("PS", "Influenza"), bivmap_title="Relation PS and Influenza")
