...
bivariate_choropleth <- function (
  ...
  # bivmap_style='jenks',# Classification type for the bins
  ...
  vp <- viewport(x=.7, y=.8, width=.2, height=.2)
  pushViewport(vp)
  print(levelplot(
    matrix(1:9, nrow=3), 
    axes=FALSE, 
    col.regions=bvColors,
    xlab=list(label=bivmap_labels[1],cex=1), 
    ylab=list(label=bivmap_labels[2],cex=1), 
    cuts=8, 
    colorkey=FALSE,
    scales=list(draw=0)),
    newpage=FALSE)
